...
helm install polyaxon/polyaxon --name=polyaxon --version=0.5.5 --namespace=polyaxon -f /tmp/polyaxon.conf
```

To upgrade polyaxon in the future you can run:
```bash
helm upgrade polyaxon polyaxon/polyaxon --version=0.5.5 --namespace=polyaxon -f /tmp/polyaxon.conf
```
