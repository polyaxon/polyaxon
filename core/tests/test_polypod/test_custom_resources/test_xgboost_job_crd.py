...
class TestXGBoostJobCRD(BaseKubeflowCRDTestCase):
    def test_get_xgb_job_custom_resource_with_no_workers(self):
        ...
        crd = get_xgb_job_custom_resource(
            namespace="default",
            resource_name="foo",
            master=None,
            worker=None,
    ...
    def test_get_xgb_job_custom_resource(self):
        termination = V1Termination(max_retries=5, ttl=10, timeout=10)
        environment = V1Environment(
            labels={"foo": "bar"},
            annotations={"foo": "bar"},
