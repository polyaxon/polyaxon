...
class V1Hook(BaseConfig, polyaxon_sdk.V1Hook):
    ...
    or trigger any logic that is tightly coupled with your a class of operations.
    ...
    When one of those events is triggered, Polyaxon will trigger the operations defined
     ...
     * done (any final state)
     ...
     You can additionally supplement the `conditions`
    ...
    After the main operation is done, conditions take advantage of information
    resolved from the main operation to decide if the hook can be started.
    ...
    In the example above, the hook will only run if the param passed is equal to "some-value".
