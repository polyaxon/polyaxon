...
class V1Build(BaseConfig, polyaxon_sdk.V1Build):
    ...
    this defining entity is instantiated.

    Build allows to dynamically recreate a new docker image
    that will be used running the main logic.
    The new image is generated automatically and set on the main container,
    ...
    The build section can resolve any context information from the main operation,
    like params, globals, ...
    ...
    based on the project name and the build operation id.
    ...
    Params can be used to pass any number of params that the component is expecting.
    ...
    Polyaxon will use by default the following destination if none is provided:
    `{{ globals.project_name }}:{{ globals.uuid }}` and will set the connection based
    on the connection field in the build process.
