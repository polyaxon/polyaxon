...
def number_percentage_format(x, rounding: int = 2):
    eps = 0.000000001
    num_format = '{0:,.0f}' if abs(int(x) - x) < eps else '{{0:,.{}f}}'.format(rounding)
...
def to_percentage(number, rounding: int = 2):
    ...
    value = number_percentage_format(number_as_int if number_as_int == rounded else rounded, rounding)
