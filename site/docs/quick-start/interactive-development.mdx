...
This page reuses the `quick-start` project from [Create a project](/docs/quick-start/#create-a-project). If you have not created it yet, complete that section before continuing.
...
Keep the run UUID printed by the command. `polyaxon ops shell` below waits for the run to reach `running`; sandbox and SSH commands expect the service to be ready.
