...
description: "Components - Become familiar with the ecosystem of Polyaxon tools with a top-level overview and useful links to get you started."
---

If you did not see the previous section of this tutorial, start with [Schedule an experiment](/docs/quick-start/schedule/).
 ...
 * From local files using the `-f` flag:
    * `polyaxon run -p quick-start -f path/to/polyaxonfile.yaml`
    * `polyaxon run -p quick-start -f path/to/polyaxonfile.yaml -f path/to/polyaxonfile_override.yaml`
 * From local Python files using `--python-module` or `-pm` flag:
    * `polyaxon run -p quick-start -pm path/to/pythonfile.py` in this case Polyaxon will look for a variable `main` which will contain your component.
    * `polyaxon run -p quick-start -pm path/to/pythonfile.py:component-name` if you have multiple components in your Python file you can specify which one to run.
 * From urls using the `--url` flag:
    * `polyaxon run -p quick-start --url=https://public-site.com/polyaxonfile.yaml` this is the command that we used to avoid cloning the project locally.
 * From a registry `--hub` flag:
    * `polyaxon run -p quick-start --hub=tensorboard:single-run` this is the command that we used to run TensorBoard.
      ...
      Please check the [Component Hub docs](/docs/registry/component-hub/overview/).
...
run:
  ...
  container:
    ...
    command: [python3, "{{ globals.artifacts_path }} + /polyaxon-quick-start/scheduling/model.py"]
```
...
Every Polyaxonfile must have a kind [component](/docs/references/polyaxonfile/specification/component/) or [operation](/docs/references/polyaxonfile/specification/operation/).
...
> Please check this section to learn more about [initializers](/docs/references/polyaxonfile/specification/init/)

In a nutshell, what Polyaxon provides is a simple way to schedule and run containerized workload.

> We will [come back](/docs/builds/overview/) to the docker image to learn how to build containers later,
...
If you open the file [model.py](https://github.com/polyaxon/polyaxon-quick-start/blob/master/scheduling/model.py)
...
<blockquote className="light">Some parts of the code were removed to reduce the size of the snippet</blockquote>
...
we can use the [inputs and outputs](/docs/references/polyaxonfile/specification/io/) sections to parametrize our program.
...
```bash
polyaxon run -p quick-start --url=https://raw.githubusercontent.com/polyaxon/polyaxon-quick-start/master/scheduling/experimentation/typed.yaml -P epochs=10 -l
```
...
List the experiments created in this project:
...
The CLI caches the last operation you executed, so you can fetch its logs without passing a run UUID:

```bash
polyaxon ops logs -p quick-start
```

This returns the logs for the last operation you executed. To fetch logs for a specific run, copy its UUID into a variable first:

```bash
export RUN_UUID=PASTE_RUN_UUID_HERE
polyaxon ops logs -p quick-start -uuid $RUN_UUID
```
...
```bash
polyaxon run -p quick-start --url=https://raw.githubusercontent.com/polyaxon/polyaxon-quick-start/master/scheduling/experimentation/typed.yaml -P lr=0.005 -P epochs=8
```


## Let's compare the experiments on the dashboard

Open the project's **Jobs** page. Use the column picker to display the output metrics you want to compare.

![select-columns](/images/dashboard/comparison/select-columns.png)

Next, add a sort field to rank the runs by a metric such as accuracy.

![comparison-sort](/images/dashboard/comparison/sort.png)

Add the metrics to the heat grid to make high and low values easier to spot.

![comparison-heat-fields](/images/dashboard/comparison/select-heat-fields.png)

The table now shows the selected metrics, sorts the runs by accuracy, and uses color to expose the differences between them.

![comparison-compare](/images/dashboard/comparison/compare.png)

To inspect how the metrics changed during each run, open the layout menu and select **Events**.

![comparison-select-events-dashboard](/images/dashboard/comparison/select-events-dashboard.png)

The Events view plots the selected runs together so you can compare their metric histories.

![comparison-charts-many](/images/dashboard/comparison/charts-many.png)

For a more detailed comparison, start a TensorBoard service for the runs. Polyaxon loads them into the same TensorBoard instance:
...
In the next section of [this tutorial](/docs/quick-start/operations/) we will explore what happens when we run an experiment or when we pass a param.
