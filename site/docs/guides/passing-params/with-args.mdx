...
This command will automatically create a [grid search](/docs/guides/scaling/hyperparameter-tuning/#grid-search) with the following matrix configuration:
...
You should notice that the CLI uses the following format: `kind:value` to pass [hyperparameters](/docs/orchestration/optimization-engine/params/), in this case it passes the `choice` kind.
Another important aspect to notice is that matrix is of kind [grid search](/docs/orchestration/optimization-engine/grid-search/) and it runs the operations sequentially by setting the `concurrency` to `1`.
...
See the intro for the hyperparameter tuning in this [section](/docs/guides/scaling/hyperparameter-tuning/) and the [optimization engine reference](/docs/orchestration/optimization-engine/).
...
We can adjust our previous polyaxonfile to run this python program, in order to avoid [uploading code](/docs/guides/iterative-process/iterate-in-notebooks/) or [cloning a repo](/docs/guides/iterative-process/iterate-with-cli-git-code/),
we will pass the python code [inline](/docs/guides/iterative-process/iterate-with-inline-scripts/):
...
> **Note**: You can learn more about how to iterate with upload, git, and inline scripts in the [iterative process introduction section](/docs/guides/iterative-process/iterate-in-notebooks/).
