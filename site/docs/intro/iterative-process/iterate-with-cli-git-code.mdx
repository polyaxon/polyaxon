...
The [init](/docs/cli/init/) command accepts both a `--git-connection` and `--git-url`. If you are using a private git repo, 
you will need to configure a [git connection](/setup/connections/git/) and redeploy Polyaxon first.
...
The last command will tell Polyaxon to look for the git configuration that we initialized earlier in this folder and detect the latest commit and inject it as a [preset](/docs/scheduling/scheduling-presets/).
