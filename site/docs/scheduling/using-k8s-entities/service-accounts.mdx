...
description: "All pods managed by Polyaxon use the default service account created during the deployment of Polyaxon CE or Polyaxon Agent (compute cluster), however you can use specific service accounts on per operation level."
---
...
please check the [service account specification](/docs/references/polyaxonfile/specification/environment/#serviceaccountname) in the environment section.
...
> we suggest that you read the minimum RBAC rules used in the default [service account](/docs/references/polyaxonfile/specification/environment/#serviceaccountname).

## Global configuration

If you to define a service account globally, we suggest creating a [preset](/docs/scheduling/scheduling-presets/overview/).
