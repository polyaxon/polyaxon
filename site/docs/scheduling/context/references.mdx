 ...
 * `{"file": [..., ...], "dirs": [..., ...]}`: An [ArtifactsType](/docs/scheduling/specification/types/#v1artifactstype) definition, and it can be used with `toInit` or can be passed to the `init` section. 
