...
description: "A feature for injecting certain information into operations at compilation time to extract repetitive configuration for node scheduling, resources requirements and definition, connections, but also for queue routing and access level control."
---
...
[environment section](/docs/references/polyaxonfile/specification/environment/), a [queue](/docs/references/polyaxonfile/specification/operation/#queue), a [container resources requirements](https://kubernetes.io/docs/concepts/containers/),
a [termination](/docs/references/polyaxonfile/specification/termination/). But oftentimes, you might want to reuse some or all of these options
...
Presets use all sections that the [operation specification](/docs/references/polyaxonfile/specification/operation/) offers, except for sections that define the component runtime, i.e.:
 * hubRef
 * dagRef
 * urlRef
 * pathRef
...
Users can just reference the presets in their [operations specification](/docs/references/polyaxonfile/specification/operation/#presets) or using the CLI/Client `polyaxon run ... --presets=preset1,preset2`

In order to create and manage scheduling presets, please check the [management section for more details](/docs/administration/organizations/presets/)
