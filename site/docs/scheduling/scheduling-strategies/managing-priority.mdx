...
description: "Prioritize important operations and enforcing preemption."
---
...
Polyaxon allows to define a priority on each queue. The priority of a queue dictates how fast will Polyaxon move your operations from the that queue and pass it to Kubernetes relative to other queues defined under each agent (compute cluster).
...
To set Kubernetes priority on an operation, you need to set the fields [priority](/docs/references/polyaxonfile/specification/environment/#priority) and/or [priority_class_name](/docs/references/polyaxonfile/specification/environment/#priorityclassname).
