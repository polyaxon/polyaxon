...
description: "Programmatic Iteration with the python client - Become familiar with the ecosystem of Polyaxon tools with a top-level overview and useful links to get you started."
---

We briefly went through one aspect of the programmatic experience in the guide [iterate-in-notebooks](/docs/iterative-process/iterate-in-notebooks/).
...
job = V1Job(
    ...
    container=V1Container(
        ...
        command=["python3", "polyaxon-quick-start/scheduling/model.py"],
        args=[
            "--conv1_size={{ conv1_size }}",
            "--conv2_size={{ conv2_size }}",
            "--dropout={{ dropout }}",
...
You can also pass a matrix definition:
...
```bash
polyaxon run -pm scheduling/experimentation/typed.py:component -P epochs=10 -l
```
...
```bash
polyaxon run -pm path/to/components.py -P epochs=10 -l
```
