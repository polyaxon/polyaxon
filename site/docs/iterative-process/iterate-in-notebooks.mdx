...
description: "Notebooks Iteration - Become familiar with the ecosystem of Polyaxon tools with a top-level overview and useful links to get you started."
---

![notebooks](/images/concepts/dashboard/notebooks.png)
...
> For more details about this command please run `polyaxon run --help`, or check the [command reference](/docs/references/cli/run/)
...
> For more details about this command please run `polyaxon ops --help`, or check the [command reference](/docs/references/cli/ops/)
...
Once the notebook is running, open the `quick-start` project in the UI, select **Services**, then open the notebook service. The **Service** tab embeds JupyterLab in Polyaxon; select **Full screen** to open a larger view.

<Frame fullWidth>
  ![JupyterLab notebook open from the Services view in Polyaxon](/images/docs/iterative-process/notebook-in-services.png)
</Frame>

You can also access the notebook from your terminal. Copy the run UUID printed when you started the notebook into a local variable:

```bash
export RUN_UUID=PASTE_RUN_UUID_HERE
```

Print both the Polyaxon service page and external service URLs:

```bash
polyaxon ops service -p quick-start -uid $RUN_UUID --url
```

Open the embedded Polyaxon service page directly:

```bash
polyaxon ops service -p quick-start -uid $RUN_UUID --yes
```

To open JupyterLab through its external service URL instead, add `--external`:

```bash
polyaxon ops service -p quick-start -uid $RUN_UUID --external --yes
```

The notebook service must be running before these commands can resolve its URL.
...
for suggestion in suggestions:
    client.create_from_url(
        url="https://raw.githubusercontent.com/polyaxon/polyaxon-quick-start/master/scheduling/experimentation/typed.yaml",
        params=suggestion)
```
...
Get metrics for the selected run
...
![run-dashboards-hiplot1](/images/dashboard/runs/programmatic-plotly-tidy.png)

![run-dashboards-hiplot1](/images/dashboard/runs/programmatic-plotly-metric.png)
