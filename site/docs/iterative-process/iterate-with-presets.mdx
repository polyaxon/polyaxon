...
description: "Using presets to improve the process of using git and code upload - Become familiar with the ecosystem of Polyaxon tools with a top-level overview and useful links to get you started."
---
 ...
 * [local code](/docs/iterative-process/iterate-with-cli-local-code/)
 * [git repo](/docs/iterative-process/iterate-with-cli-git-code/)
...
Run the commands from the root of the `polyaxon-quick-start` repository used by the previous local-code and Git guides. If you are starting here directly, complete the [local-code setup](/docs/iterative-process/iterate-with-cli-local-code/#clone-polyaxon-quick-start) first. The Git example also uses the `polyaxongit.yaml` created in the [Git guide](/docs/iterative-process/iterate-with-cli-git-code/#add-a-git-preset).
...
> **Note**: You can learn more about the [template section here](/docs/references/polyaxonfile/specification/template/)
...
In this case we use the [Git preset configured in the previous guide](/docs/iterative-process/iterate-with-cli-git-code/#add-a-git-preset) and supplement the main component with the section that resolves the path to `model.py` from the initialized code:
...
You can check the [helpers folder](https://github.com/polyaxon/polyaxon-quick-start/tree/master/scheduling/helpers) for additional preset examples.

> For more details about using presets, see the [scheduling presets guide](/docs/scheduling/scheduling-presets/overview/).
