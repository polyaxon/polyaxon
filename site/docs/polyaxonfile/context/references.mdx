 ...
 * `{"file": [..., ...], "dirs": [..., ...]}`: An [ArtifactsType](/docs/reference/polyaxonfile/specification/types/#v1artifactstype) definition, and it can be used with `toInit` or can be passed to the `init` section. 
