...
- **Asynchronous scheduling**: Operations are compiled and validated by the Compiler, then placed in queues managed by Agents (compute clusters). The Scheduler coordinates with Agents asynchronously via Redis, ensuring high spikes in submission load do not block the API or lead to timeouts.
