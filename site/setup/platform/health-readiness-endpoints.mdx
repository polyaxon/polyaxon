...
| **Agent (compute cluster)** | `healthz`   | exec (entrypoint)  | Check if the agent process is healthy                 |
...
You can override the probe settings in your Helm `config.yaml`. See the [common Helm reference](/docs/setup/platform/common-reference/) for all available options.
