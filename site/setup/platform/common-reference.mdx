...
description: "Polyaxon Common Helm Reference For Polyaxon CE and EE charts."
---
...
For EE deployments, admins can also set `singleUrl` to deploy the control plane with a single URL and automatic redirection to the right agent (compute cluster), and `defaultStrems` to set the default streams for the UI.
...
By default Polyaxon includes limits on the resources for the core components it deploys,
in order to enable/disable the resources limits, your config yaml file should include:
...
This config set the resources limits on all services.
