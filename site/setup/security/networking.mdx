...
Architecture diagram (from [architecture overview](/setup#architecture)):
...
A network load balancer is typically used to expose the gateway service and handle SSL termination ([docs](/setup/platform/encryption)).
...
The following internal connections are required within the cluster:
  ...
  * When using a [multi-namespace](/setup/deployment-strategies/strategies#multi-namespace-deployment) or [multi-cluster](/setup/deployment-strategies/strategies#multi-cluster-deployment) setup, each agent needs network connectivity back to the control plane API.
