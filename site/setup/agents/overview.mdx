---
title: "Install Polyaxon Agent (compute cluster) on Kubernetes"
description: "This is a guide to assist you through the process of setting up a Polyaxon Agent (compute cluster) deployment using Kubernetes."
---
...
<img
  src="/images/references/agent/agent-operator.png"
  alt="Polyaxon Agent gateway, streams, operator, and agent components"
  width={261}
  height={141}
  className="mx-auto h-auto w-full max-w-[261px]"
/>

 * Agents (compute clusters) interact with Polyaxon Control Plane to check the queues that they manage for operations to start, stop, or update.
...
It's important to note that Polyaxon Agent (compute cluster) is an important piece in Polyaxon's architecture to enable complete isolation of workload and artifacts and a hybrid execution and management of resources.

![polyaxon agent](/images/references/agent/agent-execution.png)
...
Polyaxon Agent ships with [default values](/docs/setup/agents/reference/), however and depending on your use case
...
You can also delete the [agent from the dashboard](/docs/administration/organizations/agents/).
