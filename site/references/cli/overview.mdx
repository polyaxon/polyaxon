...
# Polyaxon CLI Reference [#overview]
...
Aliases appear beside the canonical name, for example `project (aliases: projects)`.
Both `polyaxon project` and `polyaxon projects` accept the same subcommands and options.
This reference uses the canonical name, `project`.
...
- **Configuration**: Manage connections, presets, agents (compute clusters), and queues
