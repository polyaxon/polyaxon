...
```python
polyaxon._client.run.RunClient(owner=None, project=None, run_uuid=None, client=None, is_offline=None, no_op=None, manual_exceptions_handling=False, *, log_context=False)
```
...
When an online client uses a cached run UUID, it retains the cached identity resolved at construction.
If the cached owner or project is known and conflicts with the client's target,
accessing `run_uuid` raises `PolyaxonClientException`. Run-specific API methods access this property,
so the check happens before their request, not during client construction.
`create()` and `list()` do not consume the cached UUID and are not blocked by this check.
The same behavior applies to `AsyncRunClient`.

Context logging is disabled by default. Set `log_context=True` to log cached owner and
project values with their cache paths at construction. After validation succeeds, the first
access to a cached `run_uuid` logs its UUID and cache path once per client. Notices use INFO
through the `polyaxon.cli` logger, or WARNING when cached run ownership metadata is incomplete.
An explicit `run_uuid` produces no run-cache notice. Logging uses your existing configuration.
Set `client.log_context=False` to suppress further notices; cached ownership is still checked
on every run UUID access.
...
Changing cache files does not update an existing client's identity. Construct a new client with
explicit values, or use `set_run_uuid()` if its owner and project already match the intended run.
...
- __Properties__:
  ...
  - __status__: str.
  - __namespace__: str.
  - __client__: [PolyaxonClient](/docs/references/python-library/polyaxon-client/)
...
- __Args__:
  ...
  - __log_context__: bool, optional, default: False,
...
- __Args__:
  ...
  - __presets__: List[str], optional, the name of the
        [presets](/docs/scheduling/scheduling-presets/overview/).
...
- __Args__:
  - __last_status__: str, a valid [Statuses](/docs/references/polyaxonfile/specification/lifecycle/) value.
...
- __Args__:
  ...
  - __names__: List[str], Set[str], or comma-separated str of events to return.
...
```python
upload_artifacts(self, files, path='', overwrite=True, relative_to=None, agent=None, ignore_agent_host=False, ignore_store=None, dereference=False, recursive=True)
```
...
- __Args__:
  ...
  - __dereference__: bool, materialize symlink targets in the archive. Defaults to `False`.
  - __recursive__: bool, include directory descendants. Defaults to `True`. Directory uploads disable archive recursion after selecting files with their ignore and symlink policies.
- __Returns__:
  Upload response.
...
```python
upload_artifacts_dir(self, dirpath, path='', overwrite=True, relative_to=None, agent=None, ignore_agent_host=False, ignore_store=None, symlink_mode='skip', symlink_report_limit=20)
```
...
The default `skip` mode omits file and directory symlinks and warns with a bounded
report. `resolve-safe` uploads target contents only within `dirpath`; `resolve-all`
also includes external targets. Both resolving modes reject broken targets and
directory cycles. `error` rejects any selected symlink before upload. The API never
prompts. Ignored links are excluded before applying the mode.

This changes the previous default, which archived file symlinks as links and omitted
directory symlinks silently. Direct single-file uploads and the default archive
behavior of `upload_artifacts` are unchanged.

When an existing directory has no selected files, the method returns
`UPLOAD_SKIPPED` without making an upload request. Import this result from
`polyaxon.client` and compare it by identity:

```python
from polyaxon.client import UPLOAD_SKIPPED

result = client.upload_artifacts_dir("./code")
if result is UPLOAD_SKIPPED:
    print("No files selected; upload skipped.")
```

The method does not update the run status. The CLI treats this result as a failure
with reason `UploadEmpty` when `--sync-failure` is enabled, including for `run -u`
and mount uploads. Otherwise, it reports a skipped upload.
...
- __Args__:
  ...
  - __ignore_agent_host__: bool, optional, flag to ignore agent host
  - __ignore_store__: bool, optional, flag to ignore the ignore store and upload all files under the dirpath.
  - __symlink_mode__: str, one of `skip`, `resolve-safe`, `resolve-all`, or `error`. Defaults to `skip`.
  - __symlink_report_limit__: int, maximum links listed in a report. Defaults to 20. Use 0 for totals only.
- __Returns__:
  Upload response, or `UPLOAD_SKIPPED` when an existing directory has no selected files.
