...
description: "Params can be resolved based on literal values or can be resolved based on a reference."
---
 ...
 * `{"file": [..., ...], "dirs": [..., ...]}`: An [ArtifactsType](/docs/references/polyaxonfile/specification/types/#v1artifactstype) definition, and it can be used with `toInit` or can be passed to the `init` section.
