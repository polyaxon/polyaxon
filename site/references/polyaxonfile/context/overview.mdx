---
title: "Polyaxonfile context"
sidebarTitle: "Overview"
description: "Polyaxon provides a context to all runs to resolve information about params, id, project, ...."
---
...
The full compilation (resolution and conversion) is separated into two major steps, one on the control plane and the other on the agent (compute cluster) running on the user's cluster,
...
> **Note**: Please check the [context globals section](/docs/references/polyaxonfile/context/globals/) for more details.
...
After finishing the params section, the next section is the params defined in [joins](/docs/references/polyaxonfile/orchestration/joins/overview/).
This implies that users can leverage information coming from `globals.*` and `params.*` (or as a shortcut `*`).
In general all values resolved in the joins params are lists, unless the values are defined using the [ArtifactsType](/docs/references/polyaxonfile/specification/types/#v1artifactstype),
in which case the value is just a merged [ArtifactsType](/docs/references/polyaxonfile/specification/types/#v1artifactstype).
...
([Optimization](/docs/references/polyaxonfile/orchestration/matrix/overview/) or [Mapping](/docs/references/polyaxonfile/orchestration/mapping/overview/)).

> **Note**: Please check the [context params section](/docs/references/polyaxonfile/context/params/) for more details.
...
> **Note**: Please check the [context connections section](/docs/references/polyaxonfile/context/connections/) for more details.
