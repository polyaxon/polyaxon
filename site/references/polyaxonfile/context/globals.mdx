...
description: "This is the information that is unique to each run. Each run in Polyaxon has a project, an owner, and a user. It has as well several metadata like datetimes, e.g. time of creation, as well as information specific to the kind and runtime of the operation."
---
...
When an operation is automatically created by a [schedule](/docs/orchestration/schedules/), the globals prefix will include additionally these information:
 ...
 * The value of `param2` will be the `uuid` of the upstream run defined in the reference, only if that run with `UUID` exists and is accessible to the current operation (mostly running in the same project).
...
> **Note**: For more details about references, please check [context references section](/docs/references/polyaxonfile/context/globals/) for more details.
