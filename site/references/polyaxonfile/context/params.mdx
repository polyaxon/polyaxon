...
description: "Params Information is injected in the top level of the context and can be used without a prefix, additionally Polyaxon exposes a prefix params to provide extra information about the params context."
---

## Overview

Params provide values to the inputs and outputs definition, and can provide context only values as well.
The information provided in the params is injected after the [globals context information](/docs/references/polyaxonfile/context/globals/), which can be accessed without prefix or using the `params.*` prefix.
