---
title: Do you offer local, on-premise installs?
description: "Review Polyaxon's local, on-premises, cloud, hybrid, and air-gapped deployment options for keeping data in your environment."
tags: [billing, deployment, kubernetes]
---

# Do you offer local, on-premise installs?

Polyaxon provides several [deployment options](/docs/setup/deployment-strategies/), all of our deployment options allows to keep code, data, models, logs, and artifacts on-premise or on your cloud account.

Both Polyaxon CE and Polyaxon EE are deployed fully on your on your own machines or in your private cloud.

Polyaxon Cloud is a hybrid solution where we manage the control plane and the customer manages the data plane using one or multiple agents (compute clusters).
