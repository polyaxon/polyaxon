...
class V1ArtifactsType(BaseTypeConfig, polyaxon_sdk.V1ArtifactsType):
    ...
    >>> from polyaxon_schemas.defs.types import V1ArtifactsType
    >>> params = {
    >>>     "test1": V1Param(value=V1ArtifactsType(files=["file1", "/path/to/file2"])),
    >>>     "test2": V1Param(
    >>>         value=V1ArtifactsType(dirs=["/tensorboard-logs"]),
    >>>         connection="foo",
    >>>         to_init=True
    >>>     ),
    >>>     "test3": V1Param(
    >>>         value=V1ArtifactsType(dirs=["/"]),
