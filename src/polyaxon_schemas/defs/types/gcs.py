...
class V1GcsType(BaseTypeConfig, polyaxon_sdk.V1GcsType):
    ...
    ```python
    >>> from polyaxon_schemas.defs.types import V1GcsType
    >>> from polyaxon.polyflow import V1Param
    >>> params = {
    >>>     "test1": V1Param(value=V1GcsType(bucket="gs://bucket1")),
    >>>     "test2": V1Param(value=V1GcsType(bucket="gs://bucket1", blob="blobName")),
    >>> }
    ```
    """
