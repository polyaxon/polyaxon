 ...
 * **Enhancement**: Add configuration for `cleaner` auxiliary containers to Helm chart values. 
 ...
 * **Enhancement**: Add button to automatically redirect to the runs table filtered by the model/component version. 
 * **Enhancement**: Show latest runs by component version and their status, i.e. `project/version: √ √ √ x x √`.
 * **Enhancement**: Add discover to model/artifact/component to show runs that are not registered yet.
