 ...
 * **Enhancement**: Improve UUID references to only show 8 characters and copy on click.
 * **Enhancement**: Prevent artifacts to not refresh or re-render when the run is the running stage.
 * **Enhancement**: Improve project/component/model card with long description or multiple tags. 
 
### Commercial

 * **Enhancement**: Build and upload not setting correct lineage information.
 * **Enhancement**: Build deletion does not set the run to failed. 
 * **Enhancement**: Improve restart and resume process when an operation has build section.
