 ...
 * **Enhancement**: Show approve selection if one or multiple selected runs are pending approval.
 * **Enhancement**: Improve profile dropdown.
 * **Enhancement**: Add `reset` to the `dashboards` tab.
 * **Enhancement**: Add pending filter flag.
 * **Enhancement**: Show bookmarks filter in pin.  
