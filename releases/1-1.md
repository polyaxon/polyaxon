 ...
 * **Deprecation**: `isFlag` is deprecated in favor of the modifier `{{params.PARAM_NAME.as_arg}}` the `{{PARAM_NAME}}` will resolve to the param's value instead.
 ...
 * **Deprecation**: The `image` type does not accept a connection anymore, it only accepts the destination value as string, see the enhancement in the core section;
    * The `:latest` tag for `Kaniko` and `Dockerizer` will keep pointing to the deprecated type, but we recommend that users using Polyaxon `<= v1.8` to use the tag `:v1.1`,
