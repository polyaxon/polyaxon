 ...
 * **Enhancement**: Disable browsable API in production and allow Json render only.
 ...
 * **Enhancement**: It's possible to pass queues with the agent namespace to resolve issue when several agents have queues with the same name (non-unique queue name by organization.)
   * CLI `--queue=queue-name` the queue is resolved from default agent or `--queue=agent-name/queue-name` to specify the agent. 
   * Spec `--queue: queue-name` the queue is resolved from default agent or `--queue: agent-name/queue-name` to specify the agent.
 * **Fix**: Compiler does not raise a project does not set a list of authorized run-profiles (default behavior is to authorize all).
