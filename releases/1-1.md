 ...
 * **Enhancement**: Add support for consuming params from the deployment config file in `port-forward` command. 
 
### UI

 * **Enhancement**: Improve collapsed sidebar; reduce the delay to show tooltips faster when the sidebar is collapsed.
 ...
 * **Enhancement**: Memoize static elements to boost performance in table comparison and logs tabs.
 * **Enhancement**: Improve loading speed of the UI.
 ...
 * **Fix**: Issue with some generated labels not respecting Kubernetes the 63 limit; rfc1035/rfc1123 label (DNS_LABEL).
