...
from polyaxon.client.exceptions import AuthenticationError, PolyaxonHTTPError
from polyaxon.client.utils import create_polyaxon_tmp
from polyaxon.schemas.api.authentication import CredentialsConfig
...
class AuthApi(BaseApiHandler):
    ...
    def get_user(self, token=None):
        ...
        response = self.transport.get(
            request_url,
            headers={
                "Authorization": "{} {}".format(self.config.authentication_type, token)
            },
        )
        try:
    ...
    def _process_token(
        ...
        try:
            ...
            if response.status_code in [401, 403]:
                raise AuthenticationError(
                    request_url, response, "Invalid credentials.", response.status_code
                )
            raise AuthenticationError(
                request_url,
                response,
    ...
    def login(self, credentials, set_token=False):
        credentials = self.validate_config(
            config=credentials, config_schema=CredentialsConfig
        )
        ...
        try:
            ...
            raise PolyaxonHTTPError(request_url, None, "Connection error.", None)
        return self._process_token(
            request_url=request_url, response=response, set_token=set_token
        )

    def login_experiment_ephemeral_token(
        self,
        username,
        project_name,
        experiment_id,
    ...
    def _login_impersonate_token(
        self, request_url, internal_token, set_token=True, persist_token=True
    ):
        try:
            response = self.transport.post(
                request_url,
    ...
    def login_experiment_impersonate_token(
        self,
        username,
        project_name,
        experiment_id,
        ...
        return self._login_impersonate_token(
            request_url=request_url,
    ...
    def login_job_impersonate_token(
        self,
        username,
        project_name,
        job_id,
    ...
    def login_build_impersonate_token(
        self,
        username,
        project_name,
        job_id,
    ...
    def login_notebook_impersonate_token(
        self, username, project_name, internal_token, set_token=True, persist_token=True
    ):
        request_url = self.build_url(
            self._get_http_url("/"),
            username,
