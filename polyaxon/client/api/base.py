...
from polyaxon.schemas.specs import BaseSpecification
...
class BaseApiHandler(object):
    ...
    def prepare_results(self, response_json, config):
        return (
            config.from_dict(
                response_json, unknown=settings.RECEPTION_UNKNOWN_BEHAVIOUR
            )
            if self.config.schema_response
            else response_json
        )
    ...
    def validate_config(config, config_schema):
        if isinstance(config, Mapping):
            return config_schema.from_dict(
                config, unknown=settings.VALIDATION_UNKNOWN_BEHAVIOUR
            )
        elif not isinstance(config, config_schema):
            raise PolyaxonClientException(
                "Received an invalid config. "
                "Expects a Mapping or an instance of `{}`.".format(
