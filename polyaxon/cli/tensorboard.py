...
from polyaxon.cli.check import check_polyaxonfile, check_polyaxonfile_kind
...
def get_tensorboard_url(user, project_name, experiment=None, group=None):
    if experiment:
        return "{}/tensorboard/{}/{}/experiments/{}/\n".format(
            PolyaxonClient().api_config.http_host, user, project_name, experiment
        )
    if group:
        return "{}/tensorboard/{}/{}/groups/{}/\n".format(
            PolyaxonClient().api_config.http_host, user, project_name, group
        )
    return "{}/tensorboard/{}/{}/\n".format(
...
def url(ctx):
    ...
    if response.has_tensorboard:
        click.echo(
            get_tensorboard_url(
                user=user, project_name=project_name, experiment=experiment, group=group
            )
        )
    else:
...
def start(ctx, file):  # pylint:disable=redefined-builtin
    ...
    if experiment:
        try:
            ...
            Printer.print_error(
                "Could not start tensorboard experiment `{}`.".format(experiment)
            )
            Printer.print_error("Error message `{}`.".format(e))
            sys.exit(1)
    elif group:
