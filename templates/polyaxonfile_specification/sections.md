...
by going traversing the matrix space defined in them.

Matrix also defines constants and variable same way the `declarations` did, the only difference is
that all the values generated by the matrix contribute to the definition of a experiment group.
