...
// PolyaxonExperimentReconciler reconciles a PolyaxonExperiment object
type PolyaxonExperimentReconciler struct {
	client.Client
	Log       logr.Logger
...
// +kubebuilder:rbac:groups=core.polyaxon.com,resources=polyaxonexperiments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=core.polyaxon.com,resources=polyaxonexperiments/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=batch,resources=jobs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=batch,resources=jobs/status,verbs=get;update;patch

// Reconcile logic for PolyaxonExperimentReconciler
func (r *PolyaxonExperimentReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	ctx := context.Background()
	log := r.Log.WithValues("polyaxonexperiment", req.NamespacedName)

	// Load the instance by name
	instance := &corev1alpha1.PolyaxonExperiment{}
...
func (r *PolyaxonExperimentReconciler) reconcileJob(instance *corev1alpha1.PolyaxonExperiment) error {
	log := r.Log
	ctx := context.Background()
	...
	if !justCreated && utils.CopyJobFields(plxJob, foundJob) {
		log.V(1).Info("Updating Experment", "namespace", plxJob.Namespace, "name", plxJob.Name)
	...
	if condUpdated := r.reconcileJobStatus(instance, *foundJob); condUpdated {
		log.V(1).Info("Reconciling Experment status", "namespace", plxJob.Namespace, "name", plxJob.Name)
...
func (r *PolyaxonExperimentReconciler) reconcileJobStatus(instance *corev1alpha1.PolyaxonExperiment, job batchv1.Job) bool {
	now := metav1.Now()
	log := r.Log
	...
	if len(job.Status.Conditions) == 0 {
		if job.Status.Failed > 0 {
			instance.LogWarning("", "")
			log.V(1).Info("Experiment Logging Status Warning")
			return true
		} else if job.Status.Active > 0 {
			instance.LogRunning()
			log.V(1).Info("Experiment Logging Status Running")
			return true
		}
		return false
	}
	...
	if job.Status.Active == 0 && job.Status.Succeeded > 0 && utils.IsPlxJobSucceded(newJobCond) {
		...
		log.V(1).Info("Experiment Logging Status Succeeded")
		return true
	}
	...
	if job.Status.Failed > 0 && utils.IsPlxJobFailed(newJobCond) {
		...
		instance.Status.CompletionTime = &now
		log.V(1).Info("Experiment Logging Status Failed")
		return true
	}
	return false
}
...
func (r *PolyaxonExperimentReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&corev1alpha1.PolyaxonExperiment{}).
		Owns(&batchv1.Job{}).
		Complete(r)
}
