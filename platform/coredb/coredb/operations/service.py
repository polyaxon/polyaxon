...
class OperationsService(Service):
    ...
    def supports_kind(
        ...
        if kind not in supported_kinds and (
            not is_managed and kind not in cls.EAGER_KINDS
        ):
            raise ValueError(error_message.format(kind))
        if (
            meta_kind
            and meta_kind not in supported_kinds
            and (not is_managed and meta_kind not in cls.EAGER_KINDS)
        ):
