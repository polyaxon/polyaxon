...
class TestBaseApiHandler(TestBaseApi):
    ...
    def test_validate_config(self):
        assert isinstance(
            self.api_handler.validate_config(config={}, config_schema=BuildJobConfig),
            BuildJobConfig,
        )

        with self.assertRaises(PolyaxonClientException):
            self.api_handler.validate_config(
                config=ExperimentConfig(), config_schema=BuildJobConfig
            )
