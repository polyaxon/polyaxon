...
from polyaxon_schemas.ml.losses import MeanSquaredErrorConfig, SoftmaxCrossEntropyConfig
from polyaxon_schemas.ml.metrics import AccuracyConfig, PrecisionConfig
from polyaxon_schemas.ml.models import (
    BaseModelConfig,
    ClassifierConfig,
    GeneratorConfig,
    RegressorConfig,
...
class TestModelConfigs(TestCase):
    ...
    def test_base_model_config(self):
        config_dict = {
            ...
            "loss": SoftmaxCrossEntropyConfig(
                input_layer=["image", 0, 0], output_layer=["dense_0", 0, 0]
            ).to_schema(),
    ...
    def test_classifier_model_config(self):
        config_dict = {
            ...
            "n_classes": 10,
            "loss": SoftmaxCrossEntropyConfig(
                ["image", 0, 0], output_layer=["dense_0", 0, 0]
            ).to_schema(),
            "optimizer": AdamConfig(learning_rate=0.01).to_schema(),
            "metrics": [
                AccuracyConfig(
    ...
    def test_generator_model_config(self):
        config_dict = {
            ...
            "encoder": {
                ...
                "layers": [{"Dense": {"units": 1, "name": "encoded"}}],
            },
            "decoder": {
                "input_layers": ["image"],
                "output_layers": ["encoded"],
                "layers": [{"Dense": {"units": 1, "name": "decoded"}}],
            },
            "loss": MeanSquaredErrorConfig(
                input_layer=["image", 0, 0], output_layer=["decoded", 0, 0]
            ).to_schema(),
            "optimizer": AdamConfig(learning_rate=0.01).to_schema(),
            "metrics": [],
            "summaries": ["loss", "gradients"],
