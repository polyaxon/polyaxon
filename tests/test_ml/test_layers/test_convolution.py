...
from polyaxon_schemas.ml.constraints import MaxNormConfig
...
class TestConvolutionConfigs(TestCase):
    ...
    def assert_upsampling(upsampling_class, dim):
        config_dict = {"size": 1 if dim == 1 else [1, 1] if dim == 2 else [1, 1, 1]}
        if dim > 1:
            config_dict["data_format"] = None
    ...
    def assert_zero_padding(zero_padding_class, dim):
        config_dict = {"padding": 1 if dim == 1 else [1, 1] if dim == 2 else [1, 1, 1]}
        if dim > 1:
            config_dict["data_format"] = None
