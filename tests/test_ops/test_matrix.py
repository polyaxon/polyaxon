...
class TestMatrixConfigs(TestCase):
    ...
    def test_matrix_accept_only_one_option(self):
        config_dict = {"values": [1, 2, 3], "linspace": "1:2:1"}
        with self.assertRaises(ValidationError):
            MatrixConfig.from_dict(config_dict)
    ...
    def test_matrix_pvalues_option(self):
        config_dict = {"pvalues": [(1, 0.1), (2, 0.3), (3, 6)]}
        with self.assertRaises(ValidationError):
            MatrixConfig.from_dict(config_dict)

        config_dict = {"pvalues": [(1, 0.1), (2, 0.3), (3, 0.8)]}
        with self.assertRaises(ValidationError):
            MatrixConfig.from_dict(config_dict)

        config_dict = {"pvalues": [(1, 0.1), (2, 0.3), (3, -0.6)]}
        with self.assertRaises(ValidationError):
            MatrixConfig.from_dict(config_dict)
        ...
        config_dict = {"pvalues": [(1, 0.1), (2, 0.1), (3, 0.8)]}
        config = MatrixConfig.from_dict(config_dict)
        assert config.to_dict() == config_dict
        with self.assertRaises(ValidationError):
            config.to_numpy()
