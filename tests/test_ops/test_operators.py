...
class TestOperatorConfigs(TestCase):
    def test_for_operator_config(self):
        ...
        assert expected == config.parse(JobSpecification, Parser(), {})
    ...
    def test_if_operator_config(self):
        ...
        assert config.to_dict() == config_dict
        assert "It was True" == config.parse(JobSpecification, Parser(), {"i": 5})
        assert "It was False" == config.parse(JobSpecification, Parser(), {"i": 3})
