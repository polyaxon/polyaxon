...
class TestBookmarkApi(TestBaseApi):
    ...
    def test_get_bookmarked_builds(self):
        ...
        objs = [
            {"content_object": JobConfig(uuid=obj_uuid, project=project_uuid).to_dict()}
            for _ in range(10)
        ]
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/bookmarks", "user", "builds"
            ),
    ...
    def test_get_bookmarked_jobs(self):
        ...
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/bookmarks", "user", "jobs"
            ),
            body=json.dumps({"results": objs, "count": 10, "next": None}),
            content_type="application/json",
            status=200,
    ...
    def test_get_bookmarked_groups(self):
        ...
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/bookmarks", "user", "groups"
            ),
