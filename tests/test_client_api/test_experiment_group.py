...
class TestExperimentGroupApi(TestBaseApi):
    def setUp(self):
        ...
        self.api_handler = ExperimentGroupApi(
    ...
    def test_get_experiment_group(self):
        obj = GroupConfig(
            content="text", uuid=uuid.uuid4().hex, project=uuid.uuid4().hex
        ).to_dict()
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "username", "project_name", "groups", 1
            ),
            body=json.dumps(obj),
    ...
    def test_list_experiments(self):
        ...
        xps = [
            ExperimentConfig(
                uuid=xp_uuid, project=project_uuid, experiment_group=group_uuid
            ).to_dict()
            for _ in range(10)
        ]
        httpretty.register_uri(
        ...
        response = self.api_handler.list_experiments(
            "username", "project_name", 1, page=2
        )
        assert len(response["results"]) == 10
        assert isinstance(response["results"][0], ExperimentConfig)
