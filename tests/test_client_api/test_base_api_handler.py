...
from polyaxon.client.exceptions import ERRORS_MAPPING, PolyaxonClientException
...
class TestBaseApiHandler(TestBaseApi):
    ...
    def setUp(self):
        ...
        self.api_handler = BaseApiHandler(
    ...
    def test_get_page(self):
        ...
        assert self.api_handler.get_page(page=3) == {
            "offset": self.config.PAGE_SIZE * 2
        }
    ...
    def test_prepare_list_results(self):
        ...
        results = self.api_handler.prepare_list_results(
            response_json={"results": ["bar"]}, current_page=1, config=DummyConfig
        )
        assert results == {
            "count": 0,
            "next": None,
    ...
    def test_validate_config(self):
        assert isinstance(
            self.api_handler.validate_config(config={}, config_schema=ExperimentConfig),
            ExperimentConfig,
        )
    ...
    def test_validate_content(self):
        ...
        assert self.api_handler.validate_content("{'foo': 'bar'}") == "{'foo': 'bar'}"
        assert self.api_handler.validate_content(ExperimentConfig()) == "{}".format(
