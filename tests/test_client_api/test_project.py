...
class TestProjectApi(TestBaseApi):
    ...
    def test_create_project(self):
        ...
        httpretty.register_uri(
            httpretty.POST,
            BaseApiHandler.build_url(self.api_config.base_url, "projects"),
            body=json.dumps(obj),
            content_type="application/json",
            status=200,
        )
    ...
    def test_upload_repo(self):
        httpretty.register_uri(
            httpretty.PUT,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "user", "project", "repo", "upload"
            ),
            content_type="application/json",
            status=200,
        )
        ...
        result = self.api_handler.upload_repo(
            "user", "project", files=files, files_size=10
        )
        assert result.status_code == 200
    ...
    def test_upload_and_sync_repo(self):
        ...
        result = self.api_handler.upload_repo(
            "user", "project", files=files, files_size=10, sync=True
        )
        assert result.status_code == 200
    ...
    def test_list_experiment_groups(self):
        ...
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "user", "project", "groups"
            ),
    ...
    def test_create_experiment_group(self):
        ...
        result = self.api_handler.create_experiment_group(
            "user", "project", obj.to_dict()
        )
        assert result.to_dict() == obj.to_dict()
    ...
    def test_list_experiments(self):
        ...
        xps = [
            ExperimentConfig(uuid=xp_uuid, project=project_uuid).to_dict()
            for _ in range(10)
        ]
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "user", "project", "experiments"
            ),
        ...
        response = self.api_handler.list_experiments(
            "user", "project", metrics=True, page=2
        )
        assert len(response["results"]) == 10
        ...
        response = self.api_handler.list_experiments(
            "user", "project", params=True, page=2
        )
        assert len(response["results"]) == 10
        ...
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "username", "project_name", "experiments"
            )
        ...
        self.set_schema_response()
        response = self.api_handler.list_experiments(
            "user", "project", True, query="started_at:>=2010-10-10", sort="created_at"
        )
        assert len(response["results"]) == 10
        assert isinstance(response["results"][0], ExperimentConfig)
    ...
    def test_create_experiment(self):
        ...
        # Schema response
        self.set_schema_response()
        result = self.api_handler.create_experiment(
            "user", "project", obj_dict, group=group_id
        )
        assert result.to_dict() == obj_dict
    ...
    def test_list_jobs(self):
        ...
        xp_uuid = uuid.uuid4().hex
        xps = [
            JobConfig(uuid=xp_uuid, project=project_uuid).to_dict() for _ in range(10)
        ]
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "user", "project", "jobs"
            ),
            body=json.dumps({"results": xps, "count": 10, "next": None}),
            content_type="application/json",
            status=200,
        ...
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "username", "project_name", "jobs"
            )
        ...
        # Schema response
        self.set_schema_response()
        response = self.api_handler.list_jobs(
            "user", "project", query="started_at:>=2010-10-10", sort="created_at"
        )
        assert len(response["results"]) == 10
        assert isinstance(response["results"][0], JobConfig)
    ...
    def test_list_tensorboards(self):
        ...
        xp_uuid = uuid.uuid4().hex
        xps = [
            TensorboardJobConfig(uuid=xp_uuid, project=project_uuid).to_dict()
            for _ in range(10)
        ]
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "user", "project", "tensorboards"
            ),
            body=json.dumps({"results": xps, "count": 10, "next": None}),
            content_type="application/json",
            status=200,
        ...
        # Schema response
        self.set_schema_response()
        response = self.api_handler.list_tensorboards(
            "user", "project", query="started_at:>=2010-10-10", sort="created_at"
        )
        assert len(response["results"]) == 10
        assert isinstance(response["results"][0], TensorboardJobConfig)
    ...
    def test_list_builds(self):
        ...
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "user", "project", "builds"
            ),
            body=json.dumps({"results": xps, "count": 10, "next": None}),
            content_type="application/json",
            status=200,
        ...
        httpretty.register_uri(
            httpretty.GET,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "username", "project_name", "builds"
            )
            + "?query=started_at:>=2010-10-10,sort=created_at",
            body=json.dumps({"results": xps, "count": 10, "next": None}),
            content_type="application/json",
    ...
    def test_bookmark_project(self):
        httpretty.register_uri(
            httpretty.POST,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "username", "project_name", "bookmark"
            ),
            content_type="application/json",
            status=200,
        )
    ...
    def test_unbookmark_project(self):
        httpretty.register_uri(
            httpretty.DELETE,
            BaseApiHandler.build_url(
                self.api_config.base_url, "/", "username", "project_name", "unbookmark"
            ),
            content_type="application/json",
            status=200,
        )
