...
class TestParser(TestCase):
    ...
    def test_parse_context_expression(self):
        parser = Parser()
        assert (
            parser.parse_expression(ExperimentSpecification, "{{ something }}", {})
            == ""
        )
        assert (
            parser.parse_expression(
                ExperimentSpecification, "{{ something }}", {"something": 1}
            )
            == 1
        )
    ...
    def test_parse_graph_expression(self):
        ...
        parser = Parser()
        assert (
            parser.parse_expression(ExperimentSpecification, expression, {})
            == expression
        )
        ...
        assert (
            parser.parse_expression(
                ExperimentSpecification, expression, {}, check_graph=True
            )
            == expected_expression
        )
        ...
        expected_result = {
            "graph": {
                ...
                "layers": [
                    ...
                    {"Flatten": {"inbound_nodes": ["Conv2D_1"], "name": "Flatten_2"}},
                    {
                        "Concat": {
