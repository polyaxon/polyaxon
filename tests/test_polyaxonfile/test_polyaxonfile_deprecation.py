...
class TestPolyaxonfileDeprecation(TestCase):
    def test_simple_file_framework_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/deprecated/simple_file_framework.yml")
        )
        ...
        assert spec.environment.outputs.to_dict() == {
            "jobs": [111],
            "experiments": None,
        }
    ...
    def test_deprecated_advanced_file_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/deprecated/advanced_file.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
