...
class TestPolyaxonfile(TestCase):
    ...
    def test_non_supported_version_raises(self):
        with self.assertRaises(PolyaxonfileError):
            PolyaxonFile(
                os.path.abspath("tests/fixtures/plain/non_supported_file.yml.yml")
            )
    ...
    def test_passing_params_overrides_polyaxonfiles(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/simple_file.yml"),
    ...
    def test_passing_wrong_params_raises(self):
        with self.assertRaises(PolyaxonfileError):
            PolyaxonFile(
                os.path.abspath("tests/fixtures/plain/simple_file.yml"), params="foo"
            )

    def test_passing_debug_ttl_overrides_polyaxonfiles(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/simple_file.yml"), debug_ttl=100
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
    ...
    def test_passing_wrong_debug_ttl_raises(self):
        with self.assertRaises(PolyaxonfileError):
            PolyaxonFile(
                os.path.abspath("tests/fixtures/plain/simple_file.yml"), debug_ttl="foo"
            )
    ...
    def test_simple_file_framework_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/simple_file_framework.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
    ...
    def test_advanced_file_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/advanced_file.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
        ...
        assert (
            TensorflowSpecification.get_worker_resources(
        ...
        assert (
            TensorflowSpecification.get_ps_resources(
                environment=spec.config.tensorflow,
                cluster=cluster,
                is_distributed=is_distributed,
            )
    ...
    def test_advanced_file_with_custom_configs_and_resources_passes(self):
        ...
        assert spec.config.tensorflow.default_worker_node_selector == {"foo": True}
        assert spec.config.tensorflow.default_worker_labels == {"key1": "val1"}
        assert spec.config.tensorflow.default_worker_annotations == {"key1": "val1"}

        assert spec.config.tensorflow.worker_resources == {}
        assert spec.config.tensorflow.worker_affinities == {}
        assert isinstance(spec.config.tensorflow.worker_node_selectors[3], dict)
        assert spec.config.tensorflow.worker_node_selectors[3] == {"foo": False}
        assert spec.config.tensorflow.worker_labels[3] == {"key1": "val2"}
        assert spec.config.tensorflow.worker_annotations[3] == {"key1": "val2"}
        ...
        assert isinstance(
            spec.config.tensorflow.default_ps_resources, PodResourcesConfig
        )
        assert isinstance(
            spec.config.tensorflow.default_ps_resources.cpu, K8SResourcesConfig
        )
        ...
        assert spec.config.tensorflow.ps_tolerations[7] == [{"operator": "Exists"}]
        ...
        assert isinstance(
            spec.config.tensorflow.ps_resources[9].memory, K8SResourcesConfig
        )
    ...
    def test_wrong_grid_matrix_file_passes(self):
        with self.assertRaises(PolyaxonfileError):
            PolyaxonFile(
                os.path.abspath("tests/fixtures/plain/wrong_grid_matrix_file.yml")
            )
    ...
    def test_matrix_file_passes(self):
        ...
        assert spec.hptuning.matrix["loss"].to_dict() == {
        ...
        assert spec.framework is None
        assert spec.cluster_def == ({TaskType.MASTER: 1}, False)
        assert spec.run.cmd == "train --lr={lr} --loss={loss}".format(**spec.params)
    ...
    def test_matrix_large_n_experiments_ignored_file_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath(
                "tests/fixtures/plain/matrix_file_ignored_n_experiments.yml"
            )
        )
        spec = plxfile.specification
        spec.apply_context()
    ...
    def test_one_matrix_file_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/one_matrix_file.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
    ...
    def test_run_simple_file_passes_sdf(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/run_cmd_simple_file.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
    ...
    def test_run_simple_file_with_cmds_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/run_cmd_simple_file_list_cmds.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
    ...
    def test_run_simple_file_with_build_env_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/run_cmd_with_build_env.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
        ...
        assert spec.build.environment.node_selector == {
    ...
    def test_run_matrix_file_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/run_cmd_matrix_file.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
        ...
        assert spec.hptuning.matrix["model"].to_dict() == {
            "values": ["CDNA", "DNA", "STP"]
        }
        ...
        params["num_masks"] = 10
        assert run.cmd == (
            "video_prediction_train " '--model="{model}" ' "--num_masks={num_masks}"
        ).format(**params)

    def test_run_matrix_sampling_file_passes(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/run_cmd_matrix_sampling_file.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
    ...
    def test_distributed_tensorflow_passes(self):
        ...
        assert isinstance(
            spec.config.tensorflow.default_worker_resources, PodResourcesConfig
        )
        assert isinstance(
            spec.config.tensorflow.default_worker_resources.cpu, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.tensorflow.default_worker_resources.memory, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.tensorflow.worker_resources[3], PodResourcesConfig
        )
        assert isinstance(
            spec.config.tensorflow.worker_resources[3].memory, K8SResourcesConfig
        )
    ...
    def test_distributed_horovod_passes(self):
        ...
        assert isinstance(
            spec.config.horovod.default_worker_resources, PodResourcesConfig
        )
        assert isinstance(
            spec.config.horovod.default_worker_resources.cpu, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.horovod.default_worker_resources.memory, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.horovod.worker_resources[3].memory, K8SResourcesConfig
        )
    ...
    def test_distributed_pytorch_passes(self):
        ...
        assert isinstance(
            spec.config.pytorch.default_worker_resources, PodResourcesConfig
        )
        assert isinstance(
            spec.config.pytorch.default_worker_resources.cpu, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.pytorch.default_worker_resources.memory, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.pytorch.worker_resources[3].memory, K8SResourcesConfig
        )
    ...
    def test_distributed_mpi_passes(self):
        ...
        assert isinstance(spec.config.mpi.default_worker_resources, PodResourcesConfig)
        assert isinstance(
            spec.config.mpi.default_worker_resources.cpu, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.mpi.default_worker_resources.memory, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.mpi.default_worker_resources.gpu, K8SResourcesConfig
        )
        ...
        assert set(worker_resources.values()) == {
            spec.config.mpi.default_worker_resources
        }
    ...
    def test_distributed_mxnet_passes(self):
        ...
        assert isinstance(
            spec.config.mxnet.default_worker_resources, PodResourcesConfig
        )
        assert isinstance(
            spec.config.mxnet.default_worker_resources.cpu, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.mxnet.default_worker_resources.memory, K8SResourcesConfig
        )
        ...
        assert isinstance(
            spec.config.mxnet.worker_resources[3].memory, K8SResourcesConfig
        )
        ...
        assert isinstance(spec.config.mxnet.default_ps_resources, PodResourcesConfig)
        assert isinstance(
            spec.config.mxnet.default_ps_resources.cpu, K8SResourcesConfig
        )
        assert spec.config.mxnet.default_ps_resources.cpu.requests == 2
        assert spec.config.mxnet.default_ps_resources.cpu.limits == 4

        assert isinstance(spec.config.mxnet.ps_resources[9], PodResourcesConfig)
        assert isinstance(spec.config.mxnet.ps_resources[9].memory, K8SResourcesConfig)
    ...
    def test_build_job_with_custom_environment(self):
        plxfile = PolyaxonFile(
            os.path.abspath("tests/fixtures/plain/build_with_custom_environment.yml")
        )
        spec = plxfile.specification
        spec.apply_context()
        assert spec.version == 1
