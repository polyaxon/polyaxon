...
class TestSpecifications(TestCase):
    ...
    def test_job_specification_raises_for_missing_build_section(self):
        with self.assertRaises(PolyaxonfileError):
            JobSpecification.read(
                os.path.abspath("tests/fixtures/plain/job_missing_build.yml")
            )

    def test_job_specification_raises_for_missing_run_section(self):
        with self.assertRaises(PolyaxonfileError):
            JobSpecification.read(
                os.path.abspath("tests/fixtures/plain/job_missing_run_cmd.yml")
            )
    ...
    def test_create_tensorboard_specification(self):
        ...
        assert config["build"] == build_config
        spec = TensorboardSpecification.create_specification(
            build_config, to_dict=False
        )
        spec.apply_context()
        assert spec.build.image == build_config["image"]
    ...
    def test_create_build_specification(self):
        ...
        config = BuildSpecification.create_specification(
            run_config, config_map_refs=None, secret_refs=None
        )
        assert "config_map_refs" not in config
        assert "secret_refs" not in config

        config = BuildSpecification.create_specification(
            run_config, config_map_refs=["foo"]
        )
        spec = BuildSpecification.read(config)
        spec.apply_context()
        assert spec.parsed_data == config
        ...
        assert config["image"] == "blabla"
        spec = BuildSpecification.create_specification(
            run_config, config_map_refs=["foo"], secret_refs=["foo"], to_dict=False
        )
        spec.apply_context()
        assert spec.config.image == run_config["image"]
    ...
    def test_create_job_specification(self):
        ...
        config = JobSpecification.create_specification(
            build_config=build_config, run_config=run_config
        )
        ...
        assert config["build"] == build_config
        spec = JobSpecification.create_specification(
            build_config=build_config, run_config=run_config, to_dict=False
        )
        spec.apply_context()
        assert spec.build.image == build_config["image"]
    ...
    def test_patch_experiment(self):
        ...
        assert spec.params == params["params"]
        assert (
            spec.environment.resources.gpu.to_dict()
            == env["environment"]["resources"]["gpu"]
        )
        assert (
            spec.environment.resources.tpu.to_dict()
            == env["environment"]["resources"]["gpu"]
        )
