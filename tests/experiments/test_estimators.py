...
class TestEstimatorConstructor(test.TestCase):
    @staticmethod
    def get_dummy_model_fn(with_params=False, with_something=False):
        if with_params:
    ...
    def test_property_accessors(self):
        ...
        est = Estimator(model_fn=self.get_dummy_model_fn(with_params=True),
                        model_dir='bla', config=FakeConfig(), params=params)
    ...
    def test_default_config(self):
        ...
        self.assertTrue(isinstance(est.config, RunConfig))
    ...
    def test_model_dir_in_constructor(self):
        est = Estimator(model_fn=self.get_dummy_model_fn(), model_dir=_TMP_DIR)
    ...
    def test_model_dir_in_run_config(self):
        ...
        est = Estimator(model_fn=self.get_dummy_model_fn(), config=FakeConfig())
        self.assertIn(_TMP_DIR, est.config.model_dir)
        self.assertIn(_TMP_DIR, est.model_dir)
    ...
    def test_different_model_dir_in_constructor_and_run_config(self):
        ...
        with self.assertRaisesRegexp(
             ValueError,
             'model_dir are set both in constructor and RunConfig, but with different values'):
            Estimator(
                model_fn=self.get_dummy_model_fn(), config=FakeConfig(), model_dir=_ANOTHER_TMP_DIR)
    ...
    def test_if_params_provided_then_model_fn_should_accept_it(self):
        Estimator(model_fn=self.get_dummy_model_fn())
        with self.assertRaisesRegexp(ValueError, 'params'):
            Estimator(model_fn=self.get_dummy_model_fn(), params={'hidden_layers': 4})

    def test_not_known_model_fn_args(self):
        with self.assertRaisesRegexp(ValueError, 'something'):
            Estimator(model_fn=self.get_dummy_model_fn(with_something=True))

    def test_not_known_model_fn_args_handled_by_lambda(self):
        model_fn = self.get_dummy_model_fn(with_something=True)
        new_model_fn = lambda features, labels: model_fn(features, labels, 'something')
        Estimator(model_fn=new_model_fn)

    def test_if_model_fn_is_a_member_function_of_a_class(self):
        class ModelFnClass(object):
            def __init__(self):
                Estimator(model_fn=self.model_fn)
...
def dummy_input_fn():
    return {'x': constant_op.constant([[1], [1]])}, constant_op.constant([[1], [1]])
...
def _estimator_spec(expected_features, expected_labels, actual_features, actual_labels, mode):
    assert_ops = tuple([
...
class TestEstimatorTrain(test.TestCase):
    def test_minimal_model_fn_args(self):
        ...
        est = Estimator(model_fn=_model_fn, config=RunConfig())
        self.assertEqual(0, model_fn_call_count[0])
        est.train(input_fn=_make_input_fn(expected_features, expected_labels), steps=1)
    ...
    def test_all_model_fn_args(self):
        ...
        est = Estimator(model_fn=_model_fn, params=expected_params, config=expected_config)
        self.assertEqual(0, model_fn_call_count[0])
        est.train(input_fn=_make_input_fn(expected_features, expected_labels), steps=1)
        self.assertEqual(1, model_fn_call_count[0])
    ...
    #
    #     partial_model_fn = functools.partial(_model_fn, foo=expected_foo, bar=expected_bar)
    #
    #     est = Estimator(model_fn=partial_model_fn, params=expected_params, config=expected_config)
    #     self.assertEqual(0, model_fn_call_count[0])
    #     est.train(input_fn=_make_input_fn(expected_features, expected_labels), steps=1)
    #     self.assertEqual(1, model_fn_call_count[0])
    ...
    def test_run_train_op_and_saves_at_the_end(self):
        ...
        self.assertEqual(5, load_variable(est.model_dir, ops.GraphKeys.GLOBAL_STEP))
    ...
    def test_steps_and_saves_reloads(self):
        ...
        self.assertEqual(5, load_variable(est.model_dir, ops.GraphKeys.GLOBAL_STEP))
        est.train(dummy_input_fn, steps=5)
        self.assertEqual(10, load_variable(est.model_dir, ops.GraphKeys.GLOBAL_STEP))
    ...
    def test_checkpoint_contains_relative_paths(self):
        ...
        est = Estimator(model_dir=tmpdir, model_fn=model_fn_global_step_incrementer)
        est.train(dummy_input_fn, steps=5)

        checkpoint_file_content = file_io.read_file_to_string(os.path.join(tmpdir, 'checkpoint'))
        ckpt = checkpoint_state_pb2.CheckpointState()
        text_format.Merge(checkpoint_file_content, ckpt)
        self.assertEqual('model.ckpt-5', ckpt.model_checkpoint_path.split('/')[-1])
        self.assertAllEqual(['model.ckpt-1', 'model.ckpt-5'],
                            [path.split('/')[-1] for path in ckpt.all_model_checkpoint_paths])

    def test_train_save_copy_reload(self):
        tmpdir = tempfile.mkdtemp()
        model_dir1 = os.path.join(tmpdir, 'model_dir1')
        est1 = Estimator(model_dir=model_dir1, model_fn=model_fn_global_step_incrementer)
        est1.train(dummy_input_fn, steps=5)
        est2 = Estimator(model_dir=model_dir1, model_fn=model_fn_global_step_incrementer)
        self.assertEqual(5, load_variable(est2.model_dir, ops.GraphKeys.GLOBAL_STEP))
        est2.train(dummy_input_fn, steps=5)
        self.assertEqual(10, load_variable(est2.model_dir, ops.GraphKeys.GLOBAL_STEP))
    ...
    def test_scaffold_is_used(self):
        ...
        est = Estimator(model_fn=_model_fn_scaffold)
        est.train(dummy_input_fn, steps=1)
    ...
    def test_training_hooks_are_used(self):
        ...
        est = Estimator(model_fn=_model_fn_hooks)
    ...
    def test_chief_only_hook_should_not_be_called_on_non_chief(self):
        ...
        class NonChiefRunConfig(RunConfig):
            @property
        ...
        with test.mock.patch.object(training.SessionManager,
                                    'wait_for_session',
                                    side_effect=get_initialized_session):
            est = Estimator(model_fn=_model_fn_hooks, config=NonChiefRunConfig())
            self.assertFalse(chief_hook.begin.called)
            self.assertFalse(hook.begin.called)
            est.train(dummy_input_fn, steps=1)
            self.assertFalse(chief_hook.begin.called)
...
class EstimatorEvaluateTest(test.TestCase):
    def test_model_fn_must_return_estimator_spec(self):
        ...
        est = Estimator(model_fn=_model_fn)
        est.train(dummy_input_fn, steps=1)
        with self.assertRaisesRegexp(ValueError, 'model_fn should return an EstimatorSpec'):
    ...
    def test_scores(self):
        est = Estimator(model_fn=_model_fn_with_eval_metric_ops,
                        params={'metric_name': 'metric', 'metric_value': 2.})
        est.train(dummy_input_fn, steps=5)
    ...
    def test_global_step_metric_raises_error(self):
        est = Estimator(
            ...
            params={
                ...
                'metric_value': 2.})
        est.train(dummy_input_fn, steps=5)
        with self.assertRaisesRegexp(ValueError, 'Metric with name `global_step` is not allowed'):
            est.evaluate(dummy_input_fn, steps=1)
    ...
    def test_loss_metric_is_reported(self):
        ...
        est = Estimator(model_fn=_model_fn_with_incremental_loss)
        est.train(dummy_input_fn, steps=1)
...
class TestEstimatorPredict(test.TestCase):
    ...
    def test_predict_keys_defined_for_tensor(self):
        ...
        est.train(dummy_input_fn, steps=1)
        with self.assertRaisesRegexp(
                ValueError, 'predict_keys argument is not valid in case of non-dict predictions'):
    ...
    def test_predict_keys_does_not_exists(self):
        ...
        est = Estimator(model_fn=_model_fn)
        est.train(dummy_input_fn, steps=1)
        with self.assertRaisesRegexp(ValueError, 'Expected to run at least one output from'):
    ...
    def test_predict_from_old_model_dir(self):
        ...
        est1 = Estimator(model_fn=_model_fn)
        est1.train(dummy_input_fn, steps=1)
        est2 = Estimator(model_fn=_model_fn, model_dir=est1.model_dir)
    ...
    def test_predict_from_checkpoint_path(self):
        ...
        est2 = Estimator(model_fn=_model_fn, model_dir=est1.model_dir)
        self.assertEqual(
            [32.], next(est2.predict(dummy_input_fn,
                                     checkpoint_path=saver.latest_checkpoint(est1.model_dir))))
...
class TestEstimatorExport(test.TestCase):
    def test_export_savedmodel_proto_roundtrip(self):
        tmpdir = tempfile.mkdtemp()
        est = Estimator(model_fn=_model_fn_for_export_tests)
    ...
    def test_export_savedmodel_assets(self):
        ...
        def serving_input_receiver_with_asset_fn():
            ...
            filename = ops.convert_to_tensor(vocab_file_name, dtypes.string, name='asset_filepath')
        ...
        # Perform the export.
        export_dir_base = os.path.join(compat.as_bytes(tmpdir), compat.as_bytes('export'))
        export_dir = est.export_savedmodel(export_dir_base, serving_input_receiver_with_asset_fn)
        ...
        self.assertEqual(compat.as_bytes(_VOCAB_FILE_CONTENT),
        ...
        with ops.Graph().as_default() as graph:
            with session.Session(graph=graph) as sess:
                loader.load(sess, [tag_constants.SERVING], export_dir)
                assets = [x.eval() for x in graph.get_collection(ops.GraphKeys.ASSET_FILEPATHS)]
...
class TestEstimatorIntegration(test.TestCase):
