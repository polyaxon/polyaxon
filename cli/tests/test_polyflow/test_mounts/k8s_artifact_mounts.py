...
from polyaxon.schemas.polyflow.mounts.artifact_mounts import ArtifactMountConfig
...
class TestArtifactConfigs(TestCase):
    def test_artifact_config(self):
        ...
        config = ArtifactMountConfig.from_dict(config_dict)
        assert_equal_dict(config.to_dict(), config_dict)

        config_dict = {"name": "foo", "mount_path": 213}
        with self.assertRaises(ValidationError):
            ArtifactMountConfig.from_dict(config_dict)
