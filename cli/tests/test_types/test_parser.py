...
class TestRheaParser(TestCase):
    ...
    def test_get_auth(self):
        ...
        self.assertEqual(value, [AuthSpec("user", "pass"), AuthSpec("user2", "pass")])
        ...
        self.assertEqual(
            parser.get_auth(
                ...
                is_list=True,
                is_optional=True,
                default=[AuthSpec("user", "pass"), AuthSpec("user2", "pass")],
            ),
    ...
    def test_parse_gcs_path(self):
        ...
        # Trailing slash
        gcs_path = "gs://bucket/path/to/blob/"
        assert parser.parse_gcs_path(gcs_path) == GCSSpec("bucket", "path/to/blob/")

        # Bucket only
        gcs_path = "gs://bucket/"
        assert parser.parse_gcs_path(gcs_path) == GCSSpec("bucket", "")
