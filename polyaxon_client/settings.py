...
IS_LOCAL = config.get_boolean("POLYAXON_IS_LOCAL", is_optional=True, default=False)
IN_CLUSTER = config.get_boolean("POLYAXON_IN_CLUSTER", is_optional=True, default=False)
IS_MANAGED = config.get_boolean("POLYAXON_IS_MANAGED", is_optional=True, default=False)
POLYAXON_NO_OP_KEY = "POLYAXON_NO_OP"
NO_OP = config.get_boolean(POLYAXON_NO_OP_KEY, is_optional=True, default=False)
...
HTTP_PORT = config.get_int("POLYAXON_HTTP_PORT", is_optional=True)
if not HTTP_PORT:  # Check global config config
    HTTP_PORT = global_config.get_int("port", is_optional=True)
WS_PORT = config.get_int("POLYAXON_WS_PORT", is_optional=True)
if not WS_PORT:  # Check global config config
    WS_PORT = global_config.get_int("ws_port", is_optional=True, default=HTTP_PORT)
USE_HTTPS = config.get_boolean("POLYAXON_USE_HTTPS", is_optional=True)
...
VERIFY_SSL = config.get_boolean("POLYAXON_VERIFY_SSL", is_optional=True)
...
SECRET_EPHEMERAL_TOKEN = config.get_string(SECRET_EPHEMERAL_TOKEN_KEY, is_optional=True)
INTERNAL_TOKEN_KEY = "POLYAXON_SECRET_INTERNAL_TOKEN"  # noqa
SECRET_INTERNAL_TOKEN = config.get_string(INTERNAL_TOKEN_KEY, is_optional=True)
AUTHENTICATION_TYPE = config.get_string(
    POLYAXON_KEYS_AUTHENTICATION_TYPE,
    is_optional=True,
    default=AuthenticationTypes.TOKEN,
)
API_VERSION = config.get_string("POLYAXON_API_VERSION", is_optional=True, default="v1")
HASH_LENGTH = config.get_int("POLYAXON_HASH_LENGTH", is_optional=True, default=12)
...
INTERNAL_HEALTH_CHECK_URL = config.get_string(
    "POLYAXON_INTERNAL_HEALTH_CHECK_URL", is_optional=True
)
INTERNAL_RECONCILE_URL = config.get_string(
    "POLYAXON_INTERNAL_RECONCILE_URL", is_optional=True
)
...
LOGS_LEVEL = config.get_int(POLYAXON_KEYS_LOGS_LEVEL, is_optional=True)
