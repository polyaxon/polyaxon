...
class ExperimentGroupApi(BaseApiHandler):
    ...
    def get_experiment_group(self, username, project_name, group_id):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "groups", group_id
        )
        try:
    ...
    def update_experiment_group(
        self, username, project_name, group_id, patch_dict, background=False
    ):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "groups", group_id
        )
    ...
    def delete_experiment_group(
        self, username, project_name, group_id, background=False
    ):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "groups", group_id
        )
    ...
    def get_statuses(self, username, project_name, group_id, page=1):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "groups", group_id, "statuses"
        )
        try:
    ...
    def create_status(
        self, username, project_name, group_id, status, message=None, background=False
    ):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "groups", group_id, "statuses"
        )
        ...
        try:
            ...
            return self.prepare_results(
                response_json=response.json(), config=GroupStatusConfig
            )
        except PolyaxonClientException as e:
            self.transport.handle_exception(
    ...
    def stop(self, username, project_name, group_id, pending=False, background=False):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "groups", group_id, "stop"
        )
        json_data = None
        if pending is True:
    ...
    def bookmark(self, username, project_name, group_id, background=False):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "groups", group_id, "bookmark"
        )
        ...
        try:
            ...
            self.transport.handle_exception(
                e=e, log_message="Error while bookmarking group"
            )
            return None
    ...
    def unbookmark(self, username, project_name, group_id, background=False):
        ...
        try:
            ...
            self.transport.handle_exception(
                e=e, log_message="Error while unbookmarking group"
            )
            return None
