...
class BuildJobApi(BaseApiHandler):
    ...
    def get_build(self, username, project_name, job_id):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "builds", job_id
        )
        try:
            ...
            self.transport.handle_exception(
                e=e, log_message="Error while retrieving build"
            )
            return None

    def update_build(
        self, username, project_name, job_id, patch_dict, background=False
    ):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "builds", job_id
        )
        ...
        try:
            ...
            self.transport.handle_exception(
                e=e, log_message="Error while updating build"
            )
            return None
    ...
    def delete_build(self, username, project_name, job_id, background=False):
        ...
        try:
            ...
            self.transport.handle_exception(
                e=e, log_message="Error while deleting build"
            )
            return None

    def get_statuses(self, username, project_name, job_id, page=1):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "builds", job_id, "statuses"
        )
        try:
    ...
    def get_code_reference(self, username, project_name, job_id):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "builds", job_id, "coderef"
        )
        try:
            response = self.transport.get(request_url)
    ...
    def invalidate(self, username, project_name, job_id, background=False):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "builds", job_id, "invalidate"
        )
        if background:
    ...
    def send_logs(
        ...
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "builds", job_id, "logs"
        )
    ...
    def stop(self, username, project_name, job_id, background=False):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "builds", job_id, "stop"
        )
        ...
        try:
            ...
            self.transport.handle_exception(
                e=e, log_message="Error while stopping build"
            )
            return None

    def bookmark(self, username, project_name, job_id, background=False):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "builds", job_id, "bookmark"
        )
        ...
        try:
            ...
            self.transport.handle_exception(
                e=e, log_message="Error while bookmarking build"
            )
            return None

    def unbookmark(self, username, project_name, job_id, background=False):
        request_url = self.build_url(
            self._get_http_url(), username, project_name, "builds", job_id, "unbookmark"
        )
        ...
        try:
            ...
            self.transport.handle_exception(
                e=e, log_message="Error while unbookmarking build"
            )
            return None
    ...
    def resources(self, username, project_name, job_id, message_handler=None):
        ...
        """
        request_url = self.build_url(
            self._get_ws_url(), username, project_name, "builds", job_id, "resources"
        )
    ...
    def logs(self, username, project_name, job_id, stream=True, message_handler=None):
        ...
        if not stream:
            ...
            try:
                ...
                self.transport.handle_exception(
                    e=e, log_message="Error while retrieving builds"
                )
                return []
