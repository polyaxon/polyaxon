...
class BookmarkApi(BaseApiHandler):
    ...
    def builds(self, username, page=1):
        ...
        request_url = self.build_url(self._get_http_url(), username, "builds")
        try:
    ...
    def jobs(self, username, page=1):
        ...
        request_url = self.build_url(self._get_http_url(), username, "jobs")
        try:
            response = self.transport.get(request_url, params=self.get_page(page=page))
            return self.prepare_list_results(response.json(), page, JobConfig)
        except PolyaxonClientException as e:
    ...
    def groups(self, username, page=1):
        ...
        request_url = self.build_url(self._get_http_url(), username, "groups")
        try:
            response = self.transport.get(request_url, params=self.get_page(page=page))
