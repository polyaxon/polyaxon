...
class BaseJob(BaseTracker):
    ...
    def _set_health_url(self):
        health_url = self.client_backend.get_heartbeat_url(
            username=self.username, project_name=self.project_name, job_id=self.job_id
        )
    ...
    def log_status(self, status, message=None, traceback=None):
        self.client_backend.create_status(
            username=self.username,
            project_name=self.project_name,
            job_id=self.job_id,
            status=status,
    ...
    def log_code_ref(self):
        self.client_backend.create_code_reference(
            username=self.username,
            project_name=self.project_name,
            job_id=self.job_id,
